(defpackage #:down-graph-draw
  ...
  (:export get-rect-positions optimize-rect-by-dist rect-by-dist-drift
	   ltk-draw-rect-by-dist ltk-draw-from))
...
(defun get-rect-positions-raw (by-dist-list &key (x 10) (y 10) (dx 20) (dy 50)
			       (predicate #'true-fun))
...
(defun ltk-draw-from (from graph &key canvas (x 0) (y 0) (dx 20) (dy 50) (n 10)
     (draw-text (draw-text-default canvas))
     (draw-vert (draw-vert-default canvas))
     (draw-hor (draw-hor-default canvas dy n)) (depth 10) (predicate #'true-fun))
  (let ((rect-by-dist (get-rect-positions-raw ;Get rectangle positions.
		       (list-by-dist from graph depth)
		       :x x :y y :dx dx :dy dy :predicate predicate)))
    ;Optimize rectangle positions.
;    (optimize-rect-by-dist (cdr rect-by-dist) graph :dx dx) ;TODO doesn't seem to be working.
    ;Draw.
    (ltk-draw-rect-by-dist
     rect-by-dist graph :canvas canvas :dy dy :n n
     :draw-text draw-text :draw-vert draw-vert :draw-hor draw-hor)
    ;Kindah useless trying to return it, as the user could have done it himself.
    rect-by-dist))