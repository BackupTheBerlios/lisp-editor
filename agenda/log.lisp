...
(defun log (log-file)
  "Produces the log by evaluating the log-file."
  (with-open-file (log-stream log-file :direction :input)
    (apply #'glist 
	   (cons :series
		 (collecting ()
		   (do () 
		       ((not (when-let col (read log-stream nil nil)
			       (collecting (eval col)))) (values))))))))
...
(defun add-entry 
    (entry-args 
     &key entry-file
          gil-code (write-entry (unless entry-file
				  (lambda (stream)
				    (format stream gil-code))))
